...
public class TestGeometry extends SparkExtensionsTestBase {

  public TestGeometry(String catalogName, String implementation, Map<String, String> config) {
  ...
  private void ddl(String format) {
    ...
    Assert.assertEquals(field.dataType().typeName(), "geometry");
  ...
  private void dml(String format) {
    ...
    sql(
        ...
        tableName);
    Assert.assertEquals(spark.table(tableName).count(), 4);
    ...
    Assert.assertEquals(spark.table(tableName).count(), 3);

    // delete
    sql(
        "DELETE FROM %s WHERE ST_Intersects(ST_GeomFromText('LINESTRING(1 1, 5 5)'), geom)",
        tableName);
    Assert.assertEquals(spark.table(tableName).count(), 1);
